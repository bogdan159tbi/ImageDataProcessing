...
void task_5(FILE *out,unsigned char *img_data,unsigned char *file_info,int height,int width,int threshold_nr)
{
	...
	unsigned char **modified=malloc(height*sizeof(unsigned char*));
	if(!modified)
		return -1;

	for(int i=0;i<height;i++)
		{modified[i]=malloc(width*sizeof(unsigned char));
		if(!modified[i])
			{
			for(int j=0;j<i;j++)
				free(modified[j]);
	...
	for(int i=0;i<height;i++)
		...
		{
		if(modified[i][j]==0)
			{int nr=0,sumr=0,sumg=0,sumb=0;
			struct Queue* queueX=create(width*height);//sa modific cu ceva mai mare daca e
			struct Queue* queueY=create(width*height);		
			for(int ii=i;ii<height;ii++)
			  for(int jj=j;jj<width;jj++)
				if(conditie_task5(matrice_red[i][j],matrice_green[i][j],matrice_blue[i][j],matrice_red[ii][jj],matrice_green[ii][jj],matrice_blue[ii][jj],threshold_nr) && !modified[ii][jj])
				{
				nr++;
				sumr+=matrice_red[ii][jj];
				sumg+=matrice_green[ii][jj];
				sumb+=matrice_blue[ii][jj];
				add_queue(queueX,ii);
				add_queue(queueY,jj);
				modified[ii][jj]=1;
				}
			for(int ii=0;ii<queueX->size;ii++)
				{
					int x=front_queue(queueX);
					remove_queue(queueX);
					int y=front_queue(queueY);
					remove_queue(queueY);
					matrice_red[x][y]=nr/sumr;
					matrice_blue[x][y]=nr/sumb;
					matrice_green[x][y]=nr/sumg;
				}
			}
		}
	/*
	int directie_X[8]={-1,-1,0,1,1,1,0,-1};
	int directie_Y[8]={0,1,1,1,0,-1,-1,-1};
	struct Queue* queueX=create(width*height);//sa modific cu ceva mai mare daca e
	struct Queue* queueY=create(width*height);
	int x_start=0,y_start=0;
	modified[x_start][y_start]=1;
	add_queue(queueX,x_start);
	add_queue(queueY,y_start);

	while(!full_queue(queueX) && !full_queue(queueY))
	{	
		
		int i=front_queue(queueX);
		int j=front_queue(queueY);
			for(int directie=0;directie<8;directie++)
			{
			int nextX=i+directie_X[directie];
			int nextY=j+directie_Y[directie];
			if(okXY(nextX,nextY,height,width))
			if(conditie_task5(matrice_red[x_start][y_start],matrice_green[x_start][y_start],matrice_blue[x_start][y_start],matrice_red[nextX][nextY],matrice_blue[nextX][nextY],matrice_blue[nextX][nextY],threshold_nr))
				{
				modified[i][j]=1;
				add_queue(queueX,nextX);
				add_queue(queueY,nextY);
				}
			}
...
int main(int argc, char const *argv[])
{
	...
	threshold_name[strlen(threshold_name)]='\0';
	...
	fclose(cluster);
	out=fopen(output_task5,"rb");
	task_5(out,img_data,file_info,height,width,threshold_nr);
	*/
	for(int i=0;i<dim_filter;i++)
